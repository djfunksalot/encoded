...
import { computeAssemblyAnnotationValue } from '../filegallery';
...
/**
 * Sorting function to sort an array of assemblies according to the order specified in
 * `assemblySortOrder`.
 * @param {array} assemblyList Array of assembly facet terms to sort
 */
 ...
 *    sorter: function to sort facet terms within the facet
 */
const displayedFacetFields = [
    { field: 'assembly', radio: true, sorter: assemblySorter },
    { field: 'output_type' },
    { field: 'file_type' },
...
/** File facet fields to request -- superset of those displayed */
...
/** Includes facet fields with radio-button choices for its terms instead of checkboxes */
const radioFacetFields = ['assembly'];
...
class FacetTerm extends React.Component {
    ...
    render() {
        const { term, termCount, maxTermCount, selected, FacetTermSelectRenderer } = this.props;
        return (
...
const requestFacet = (elements, fetch, queryString, session) => {
    ...
    // endpoint instead.
    const fieldQuery = requestedFacetFields.reduce((query, facetField) => `${query}&field=files.${facetField.field}`, '');
    return sessionPromise.then(csrfToken => (
        fetch(`/search_elements/type=Experiment${fieldQuery}&field=files.restricted&field=files.@id&limit=all&filterresponse=off${queryString || ''}`, {
            method: 'POST',
            headers: {
...
const addToAccumulatingFacets = (accumulatingResults, currentResults, facetFields) => {
    ...
    if (currentResults['@graph'] && currentResults['@graph'].length > 0) {
        ...
        facetFields.forEach((facetField) => {
            const matchingFacetIndex = fileResults.facets.findIndex(facet => facet.field === facetField.field);
            ...
            fileFacetsRefs[facetField.field] = fileResults.facets[matchingFacetIndex];
        });
            ...
            if (experiment.files && experiment.files.length > 0) {
                experiment.files.forEach((file) => {
                    if (!file.restricted) {
                        ...
                        facetFields.forEach((facetField) => {
                            const fileFieldValue = getObjectFieldValue(file, facetField.field);
                            if (fileFieldValue) {
                                const facet = fileFacetsRefs[facetField.field];
...
class Facet extends React.Component {
    ...
    render() {
        ...
        const FacetTermSelectRenderer = radioFacetFields.includes(displayedFacetField.field) ? FacetTermRadio : FacetTermCheck;
        return (
            <div className="facet">
...
class FileFacets extends React.Component {
    ...
    render() {
        const { facets, selectedTerms, selectedFileCount, termClickHandler, facetLoadProgress } = this.props;
...
FileFacets.propTypes = {
    /** Array objects for each displayed facet */
    facets: PropTypes.array,
    /** Selected facet fields */
    selectedTerms: PropTypes.object,
    /** Count of the file selected by current selected facet terms */
...
const createInitialCartState = () => {
    const newState = {
        ...
        /** Number of files selected for download */
        selectedFileCount: 0,
        /** Array of visualizable file objects, filtered by selected facets */
        visualizableFilesFiltered: [],
        /** Tracks facet-loading for loading progress bar */
        facetLoadProgress: null,
    };
    displayedFacetFields.forEach((facetField) => {
        newState.selectedTerms[facetField.field] = [];
    });
    return newState;
};
...
class CartComponent extends React.Component {
    constructor() {
        ...
        /** Array of all displayable facets directly from file search results */
        this.fileFacets = {};
        displayedFacetFields.forEach((facetField) => {
            this.fileFacets[facetField.field] = [];
        });
    ...
    retrieveFileFacetTemplate() {
        return this.context.fetch(
            ...
            // Extract only the facets defined in displayedFacetFields to make an array of facet
            // objects containing only what we need to render the file facets, and with no facet
            // terms and zero totals. The subsequent requests will fill all these in.
            const facets = [];
            displayedFacetFields.forEach((facetField) => {
                const displayedFacet = response.facets.find(facet => facet.field === facetField.field);
                if (displayedFacet) {
                    facets.push(Object.assign({}, displayedFacet, { terms: [], total: 0 }));
                }
            });

            // To start the facet accumulation process, use the empty search results and overwrite
            // the facets with the empty ones we generated above, and set the total to 0. They'll
            // get filled in during the next step.
    ...
    retrieveFileFacets() {
        ...
        displayedFacetFields.forEach((facetField) => {
            if (this.state.selectedTerms[facetField.field].length > 0) {
                const termQuery = this.state.selectedTerms[facetField.field].map(term => `files.${facetField.field}=${encoding.encodedURIComponentOLD(term)}`).join('&');
        ...
        chunks.reduce((promiseChain, currentChunk, currentChunkIndex) => (
            ...
            this.allFilesPartial = accumulatedResults['@graph'];
            this.titleMap = {};
            accumulatedResults.facets.forEach((facet) => {
                this.titleMap[facet.field] = facet.title;
            });

            // Update the file facet array state, the list of viewable dataset @ids, and also
            // indicate that the file facet loading progress bar can disappear.
            const newFacets = Object.assign({}, this.state.fileFacets);
            accumulatedResults.facets.forEach((facet) => {
                const displayedFacetField = displayedFacetFields.find(facetField => facetField.field === facet.field);
                if (displayedFacetField) {
                    newFacets[facet.field] = displayedFacetField.sorter ?
                        displayedFacetField.sorter(facet.terms, 'key')
                    :
                        _(facet.terms).sortBy(facetTerm => facetTerm.key.toLowerCase());
                }
            });
            this.setState({ fileFacets: newFacets, viewableElementCount, viewableElements, facetLoadProgress: -1 });

            // For any radio button facets, select the first term. First get the field values of
            // all radio-button facets.
            const newSelectedTerms = {};
            displayedFacetFields.forEach((displayedFacetField) => {
                if (displayedFacetField.radio && this.state.selectedTerms[displayedFacetField.field].length === 0 && newFacets[displayedFacetField.field].length > 0) {
                    newSelectedTerms[displayedFacetField.field] = [newFacets[displayedFacetField.field][0].key];
                } else {
                    newSelectedTerms[displayedFacetField.field] = this.state.selectedTerms[displayedFacetField.field].slice(0);
                }
            });
    ...
    assembleFacets(selectedTerms) {
        ...
        if (this.allFilesPartial.length > 0) {
            const localSelectedTerms = selectedTerms || this.state.selectedTerms;
            const selectedFacetKeys = Object.keys(localSelectedTerms).filter(term => localSelectedTerms[term].length > 0);
            this.allFilesPartial.forEach((file) => {
                ...
                let match = selectedFacetKeys.every((selectedFacetKey) => {
                    ...
                    return localSelectedTerms[selectedFacetKey].indexOf(filePropValue) !== -1;
                });
                ...
                if (match) {
                    ...
                    Object.keys(localSelectedTerms).forEach((facetField) => {
                    ...
                    selectedFacetKeys.forEach((selectedFacetField) => {
                        ...
                        match = filteredSelectedFacetKeys.every((filteredSelectedFacetKey) => {
                            ...
                            return localSelectedTerms[filteredSelectedFacetKey].indexOf(filePropValue) !== -1;
                        });
            ...
            Object.keys(localSelectedTerms).forEach((field) => {
                const matchingFacet = facets.find(facet => facet.field === field);
                if (matchingFacet) {
                    ...
                    const missingTerms = localSelectedTerms[field].filter(term => matchingFacetTerms.indexOf(term) === -1);
            ...
            facets.forEach((facet) => {
                // While we're looping through facets, also set its title.
                facet.title = this.titleMap[facet.field];
        ...
        return { facets: facets.length > 0 ? facets : null, selectedFileCount };
    }
    ...
    handleTermClick(clickedField, clickedTerm) {
        this.setState((prevState) => {
            const newSelectedTerms = {};
            if (radioFacetFields.includes(clickedField)) {
            ...
            return { selectedTerms: newSelectedTerms, facets, selectedFileCount };
        }, () => {
            this.loadPageFiles();
            this.setState({ currentGenomeBrowserPage: 0 });
        });
    }
    ...
    loadPageFiles() {
        ...
        const visualizableFilesPartial = filterForVisualizableFiles(this.allFilesPartial).filter(file => (
            Object.keys(this.state.selectedTerms).every(term => (
                this.state.selectedTerms[term].length > 0 ? this.state.selectedTerms[term].includes(getObjectFieldValue(file, term)) : true
            ))
        ));

        if (visualizableFilesPartial.length > 0) {
            const totalGenomeBrowserPages = Math.floor(visualizableFilesPartial.length / PAGE_FILE_COUNT) + (visualizableFilesPartial.length % PAGE_FILE_COUNT !== 0 ? 1 : 0);
            ...
            const currentPageFilesPartial = visualizableFilesPartial.slice(pageStartingIndex, (pageStartingIndex + PAGE_FILE_COUNT) - 1);
    ...
    render() {
        ...
        // Generate pager component for the currently selcted tab.
        let currentPager;
        if (this.state.currentDisplay === 'datasets') {
